...
const server = http.createServer((req, res) => {
  ...
  res.end('Hola Mundo, ejecucion desde my_app clienWeb2!\n');
});
...
cat <<EOF | sudo tee /etc/consul/clientWeb2-service.json >> "$LOG_FILE" 2>&1
{
  "services": [
    {
      "Name": "my_app",
...
consul services register /etc/consul/clientWeb2-service.json >> "$LOG_FILE" 2>&1